...
| 話し中 | ✅ ある | `vc.js` が AnalyserNode で自分のマイクと各ピアの受信音声の音量を測り、話し中の枠に `.vc-peer-speaking`（金色の縁）とマイクマークの点灯を出す。AudioContext を使えない環境では表示が出ないだけで会話は従来どおり |
| **他人の 声入／声切** | ✅ ある | `rtcSignal` の payload に `{kind:"mic", muted}` を相乗りさせて相互に告知する（`vc.js`）。**サーバーは payload を解釈せず中継するだけなので、`S2C` の追加は不要だった。** 各枠の右下にマイクマークが出る |
...
4. **型の追加＝チーム合意が要るもの** … サムネイル画像、絞り込みピル（趣味タグ・ルームタグ・合言葉・ランダムマッチ・ノック・あだ名のアカウント保存は実装済み）
