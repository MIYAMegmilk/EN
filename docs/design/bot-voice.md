...
- 状態: **bot 側・クライアント側は実装済み。サーバー側（types.ts / main.ts / rooms.ts）はちいかわへ依頼中**（§5）
...
**あわせて報告（別件のバグ）**: `C2S_TYPES` に `"setBot"` と `"endPollVote"` が入っていない。
types.ts の C2S と rooms.ts のハンドラには両方あるので、**実際の WebSocket 経由では
bot の ON/OFF と終了アンケートの投票が `asC2S` で落ちて INVALID_INPUT になる**（§3.10 が動かない）。
`rooms_bot_test.ts` は manager を直接叩いていて main.ts を通らないため気づけていない。
`"voice"` と同じ場所に3行足してほしい。
...
**判断してほしい点**: 文字起こしを VC 参加者（`isVcEligible`）に限るかどうか。
VC 枠外の人が偽の `voice` を投げても実害はチャット欄が汚れる程度だが、
`isVcEligible` で絞るほうが素直だと思う。ここはちいかわの判断に従う。
...
S2C `voice` の表示は chat.js に混ぜず、字幕行か専用のログに出す想定
（チャット欄に流すと §3.9 の会話が読めなくなる）。ここもちいかわの判断に従う。
...
- S2C `voice` の表示場所（字幕行 / 専用ログ / チャット混在）→ ちいかわ判断
