# 場回し bot ― 表示と川柳判定（設計書）

- 版: v1.0
- 日付: 2026-08-24
- 担当: ひろし（bot）
- 前提: 詳細仕様書 [overall.md](../spec/overall.md) v0.5 §3.10、全体設計書 [overall.md](overall.md) v1.0
- 関連: 通話の文字起こしは [bot-voice.md](bot-voice.md)
- 状態: **実装・画面への差し込みとも完了**（差し込みは #12 で ちいかわ が対応、§4）。
  **しゅんぴが一度も動いていなかった問題も解消**（あだ名の省略を画面から使えるようにした。§5）

---

## 1. 何を解いたか

bot のロジック（`server/bot.ts`）とルーム配線（`server/rooms.ts`）は先に入っていたが、
**受け取る側がいなかった**ため、実際の画面では bot が半分しか働いていなかった。

| 症状 | 原因 |
|---|---|
| せりが川柳を拾ってもテロップが出ない | `ChatMessage.card` を描画する実装がどこにもない |
| ゲーム提案を押せない | 同上（カードがただの文字列として流れる） |
| **終了アンケートに投票できない** | `endPollVote` を送る UI が存在しない |
| bot の ON/OFF ができない | `setBot` を送る UI が存在しない |
| **漢字混じりの川柳を拾えない** | kuromoji プロバイダが未配線（かなのみ） |

前4つを `public/room/bot.js`（新規）で、最後の1つを kuromoji の配線で解いた。

---

## 2. `public/room/bot.js`（実装済み）

`chat.js` / `vc.js` / `voice.js` と同じ classic script。`Bot` だけをグローバルに公開する。
DOM は空のコンテナに自前で組み立てるので、呼び出し側が用意するのは `<div id="bot"></div>` 1つでよい。

```
#bot
 ├ .bot-toggles  … しゅんぴ / せり / ぐっちー の ON/OFF
 ├ .bot-stage    … 直近のテロップ（川柳 / ゲーム提案 / アンケート結果）
 └ .bot-poll     … 集計中の終了アンケート（投票ボタン + 残り時間）
```

### 責務の線引き

- **bot の発話そのものはチャット欄に出る**（`chat.js` の仕事）。bot.js が持つのは演出面だけ
- **読み上げ（音声合成）はしない**。bot の発話はチャットのみ（§3.10 / 全体設計書 §3）。
  回帰よけに「`speechSynthesis` に触れていないこと」のテストを置いてある

### API

```js
Bot.init({ send, container, onError });
Bot.setSelfId(playerId);      // hostChanged の判定に使う
Bot.handleServerMessage(msg); // roomState / chat / botState / botPollClosed / hostChanged / kicked
Bot.reset();                  // 退室時
Bot.getState();               // デバッグ・テスト用
```

### 決めごとと理由

| 決めごと | 理由 |
|---|---|
| トグルは**ホスト以外にも見せる**（操作だけ不可） | 「いま誰が動いているか」は全員が知りたい。隠すと bot が黙った理由が分からない |
| ON/OFF は**楽観更新しない**（`botState` を待つ） | サーバーが正。押した直後に見た目だけ変わると、拒否されたとき食い違う |
| アンケートは**投票し直せる** | サーバーは最後の票を採る。締切までは気が変わってよい |
| `botPollClosed` は **pollId を照合**してから閉じる | 遅れて届いた前回の結果で、次のアンケートを消さないため |
| 再接続時は `snapshot.botPoll` と履歴の直近カード1枚を**復元** | 途中入室・再接続でも投票に参加できる（§3.2） |
| カウントダウンは締切で**必ず止める** | タイマーを残さない。締めるのはサーバーで、こちらは表示を止めるだけ |
| 描画は `textContent` のみ | §3.8。句・あだ名・ゲーム名はすべてユーザー由来 |

## 3. kuromoji の配線（実装済み）

かなプロバイダだけでは**漢字が1文字でも混ざると拾えない**。実測:

```
○ ふるいけやかわずとびこむみずのおと   → 検出できる
✕ 古池や蛙飛び込む水の音               → 検出できない
```

音声認識の出力は漢字かな混じりで返ってくるので、[bot-voice.md](bot-voice.md) の文字起こしは
これがないと実質発火しない。

`server/senryu.ts` に `createSenryuDetector()` を足し、`server/main.ts` で `RoomManager` に渡した。

**「使うまで読まない、読めたら差し替える」**方式にしている。

- 最初の判定要求で辞書の読み込みを**開始する**（プロセスで1回だけ）
- 読み終わるまでは かな のみで判定する（`bot.reduce` は純粋関数で `await` できないため待たせない）
- 読み終わったら `[kuromoji, kana]` で判定する（kuromoji はかなだけの文で解析が破綻するので、
  うしろに kana を残して互いの穴を埋める。`detectSenryuAny` のコメント参照）
- 読み込めない環境（辞書なし・npm 不通）では かな のまま動き続ける

一度も発言のないルームや、川柳判定を使わないテストでは辞書を読まない。

### メモリの申し送り（既定 ON のまま、逃げ道を足した）

実測（Deno 2.9 / Windows）: **初期化 390〜560ms・辞書 17MB**。

常駐量は測り直した。`startServer` に接続を1本つないで発言1回、を5回:

```
RSS 57MB → 384MB / 277MB / 384MB / 277MB / 381MB
```

**+220〜330MB の2値に振れる**（GC のタイミング。以前 +385MB と書いたのは単体計測の上振れ）。
§6 には余裕を見て **400MB を見込む**と書いた。
4GB に対して約 10% で、**ルーム数に比例しない一度きりの定数**である。

`senryu.ts` のコメント自身が「本番投入は §6 のメモリ見積りとあわせてチーム合意のうえで」と
書いているのに、初版は合意を取らないまま無条件で有効になっていた。**足りなかったのは
見積りの開示であって、機能そのものではない**と判断し、次のようにした。

- **既定は有効のまま**。かなのみに倒すと せり が漢字混じりを拾えず、
  [bot-voice.md](bot-voice.md) の文字起こしが実質発火しない（音声認識の出力は漢字かな混じり）。
  ①と同じ「実装済みなのに動かない」を作るほうが痛い
- **§6 に見積りを書き出して ack を求める**。これが本来やるべきだったこと
- **逃げ道だけ環境変数に出した**。`EN_SENRYU_KUROMOJI=0`（`false` / `off` / `no` も可）で
  かなのみに戻る。メモリが問題になったときコード変更・再デプロイなしで倒せる

```
未設定 / 上記以外の値  → kuromoji を読み込む（既定。漢字混じりも拾う。常駐 +220〜330MB）
EN_SENRYU_KUROMOJI=0  → かなのみ（漢字が1文字でも混ざると拾えない）
```

- 判定は `main.ts` の `useKuromojiSenryu()`。`.env` でも環境変数でもよい（`buildIceServers` と同じ読み方）
- **意図の読めない値では倒さない**。`disable` などと書いても有効のまま。
  誤記で機能が黙って死ぬより、明示的に倒したときだけ倒れるほうがよい
- どちらのモードで動いているかは起動時にログへ出る
- `createSenryuDetector({ kuromoji })` が唯一の分岐点。false なら辞書を一切読まず `onReady` も呼ばれない

**宿題**: §6 の ack が取れたら `deno.json` の imports へ `npm:kuromoji@0.1.2` を移す。
いまは未合意の共有設定を触らないよう動的 import のままで、`deno.lock` にだけ載っている。
既定 ON である以上これは本番依存なので、明示したほうがよい。

## 4. 画面への差し込み（#12 で完了）

`public/index.html` と `public/app.js` は担当外なので触らず、この §4 に必要な差分を書いて
依頼として出していた。**2026-08-24、ちいかわが #12 で対応済み**（`0008625`
「feat: voice.js と bot.js を画面に配線する（§3.6 / §3.10）」）。voice.js も同時に配線された。

入ったもの:

| 場所 | 内容 |
|---|---|
| `index.html` | `app.js` より前に `<script src="./room/bot.js"></script>` |
| `index.html` | `#vc` と `#phase` のあいだに `<section id="bot" class="card block hidden"></section>` |
| `app.js` | 受信ループの `Bot.handleServerMessage` / `roomState` の `Bot.setSelfId` |
| `app.js` | 退室・キックの `Bot.reset` / `renderAll` の表示切替 / `Bot.init({ send, container: $("bot"), onError: showError })` |
| `en.css` | `bot-*` のスタイル（川柳は縦積み） |

**前提（解決済み）**: `main.ts` の `C2S_TYPES` に `setBot` / `endPollVote` が入っておらず、
bot.js が正しく送っても `asC2S` で落ちる状態だったが、**#10 で修正済み**。
`types.ts` の `C2S` 型と `C2S_TYPES` を双方向で照合するテストも入ったので、
今後この種の取りこぼしは検出される。

### 残っている小さな穴（体裁のみ）

`bot.js` が付けるクラスのうち、次の4つが `en.css` に未定義:

```
.bot-card-game  .bot-card-poll-result  .bot-card-senryu  .bot-poll-button
```

いずれも `.bot-card` か `.btn` が併記されているので壊れはしない。実害があるのは
`.bot-card-game` だけで、ゲーム提案カードのタイトルと「これで遊ぶ」ボタンが横並びにならず
縦積みになる。`en.css` は担当外なので、直すなら依頼の形になる。急ぎではない。

## 5. あだ名の省略を画面から使えるようにした

**しゅんぴが一度も動いていなかった。** `pickNickname` も `NAMING_TEXTS` も配線済みで、
サーバーは `join` の `nickname` 省略を受け付けるのに、**画面から省略する手段がなかった**。

```ts
// types.ts（もともと実装済み）
t: "join";
/**
 * あだ名。省略するとサーバーがしゅんぴの二つ名を付ける（§3.0 / §3.10）。
 * 空文字は「入力し忘れ」と区別できないので従来どおりエラーにする
 */
nickname?: string;
```

`rooms.ts` も `msg.nickname === undefined` なら `pickNickname` を呼ぶ実装が入っていた。
**`validateNickname` は変えていない。** 空文字を弾く仕様はそのままで正しく、
足りなかったのは「そもそも送らない」経路だった。

### 直した4か所

`public/` は担当外だが、依頼ではなく実装する判断（ひろし）。差分はいずれも小さい。

| 場所 | 変更 |
|---|---|
| `app.js` の入店ボタン | 空欄なら `nickname` を積まない。`{ t, roomCode }` だけ送る |
| `app.js` の `ws.onopen` | 再接続でも空欄なら積まない。**これを直さないと、あだ名なしで入った人が復帰できない** |
| `rooms.js` の `enterRoom` | 空あだ名で入店を止めるガードを削除（`els.nickname` も未使用になるので落とした） |
| `index.html` | ラベルを「20文字以内・空欄可」に。placeholder に「空欄ならこちらで付けます」 |

再接続を直す必要があるのは、`doJoin` が session で復帰したら早期 return して**あだ名を見ない**ため。
旧コードは `nickname.length > 0` を条件に再接続そのものを送っていなかったので、
あだ名を省略して入室した人は WS が切れた瞬間に復帰できなくなるところだった。

`createRoom` は**必須のまま**にした。ルーム作成はログイン必須で、ログイン済みなら
プロフィールのあだ名が自動入力される（`app.js` の `refreshAccount`）ため空欄で作る場面がほぼなく、
必須を外すと `C2S` の型変更＝チーム合意が要る割に見返りが小さい。

### 確かめたこと

クライアント側はテスト網の外なので、実際にサーバーを立てて確認した。

```
あだ名 空欄 → {"t":"join","roomCode":"123456"}              （nickname キーごと無い）
あだ名 あり → {"t":"join","roomCode":"123456","nickname":"ひろし"}  （前後の空白は落ちる）
```

サーバー側は通しで確認した。

```
省略で入室 → 付いたあだ名: たそがれカピバラ
しゅんぴの発話: 「たそがれカピバラ」さん、いらっしゃい。あだ名はこちらで勝手につけました
空文字での入室: INVALID_INPUT / ニックネームを入力してください   （従来どおり）
全員のあだ名: ひろし / たそがれカピバラ / ほろよいレッサーパンダ  （衝突しない）
```

### 仕様書の追随（§7 の改訂依頼に含める）

- **§3.1**: 「ニックネームは全画面共通で 20 文字以内（ゲスト・ログイン済みとも入室時に入力）」
  が必須と読める。**入室時は省略可**で、省略するとサーバーが二つ名を付けることを書きたい
- **§3.10**: 発話トリガーの一覧に**しゅんぴの命名がそもそも載っていない**
  （挨拶・沈黙検知・ゲーム提案・結果への相槌の4つだけ）。実装が先行している

## 5.5. 沈黙判定を 3 分 → 30 秒にした

`SILENCE_MS` を `3 * 60_000` から `30_000` に変更した（`server/bot.ts`）。
3 分は呑み会の間としては長すぎて、会話が途切れてから話題カードが飛んでくる頃には
場が冷えきっている。ぐっちーの話題カードと、なべの `stuck` 判定（ゲーム提案・
終了アンケート）がまとめて 30 秒起点になる。

### 副作用として出た2点

1. **`BOT_TICK_MS`（60 秒）に負けている** … `rooms.ts` の tick が 60 秒周期なので、
   しきい値を 30 秒にしても判定が走るのは 60 秒ごと。実際の発火は無言開始から
   **30〜90 秒後にばらける**。30 秒という値を額面どおり効かせるには
   `BOT_TICK_MS` を 15〜30 秒に縮める必要がある。`rooms.ts` は担当外なので
   §7 でちいかわに依頼する
2. **`LOBBY_QUIET_MS`（60 秒）が実質死んだ** … 「直近 60 秒は無言」を満たす時点で
   沈黙判定（30 秒）はとっくに成立しているので、`tickBots` はぐっちーの話題カードで
   早期 return してしまい、`lobbyStale` の枝までほぼ届かない。届くのは
   「ぐっちーが発話枠を使い切っていて、かつ `silenceStreak` がまだ 2 未満」という
   狭い場合だけで、それ以外は `stuck` に吸収される。
   `lobbyStale` を活かすなら `LOBBY_QUIET_MS` を `SILENCE_MS` より短くする必要がある

### テストへの影響

`SILENCE_MS * i` で時間を進めていたループを、固定値の `SILENT_STEP`（3 分）に置き換えた。
しきい値を縮めるとテストの時間軸まで一緒に縮み、`BOT_RATE_WINDOW_MS`（10 分窓）も
`END_POLL_MIN_AGE_MS`（45 分）も跨げずに「なべがそもそも動かない」ため。
しきい値の境界そのものを見るテストだけが `SILENCE_MS` を参照する。
`deno task test` → **1081 passed / 0 failed**。

## 5.6. あだ名を趣味タグから連想させた

しゅんぴの `pickNickname` は **形容 12 × 動物 12 = 144 通りの直積からランダムに 1 つ引くだけ**で、
その人の情報を一切見ていなかった。誰が入っても「たそがれカピバラ」で、二つ名が
会話のとっかかりにならない。§3.11 の趣味タグが卓まで通った（`Player.tags`）ので、
**入室者本人のタグから連想した語で組み立てる**ようにした。

```
reading            → よふかしフクロウ
alcohol + camping  → ほろよいとっくり
タグなし            → たそがれカピバラ（従来どおり）
```

外部 AI は使わない（§3.10）。連想はすべて `bot_templates.ts` の静的な語彙表
`TAG_NICKNAME_WORDS`（タグID → 形容3語・名詞3語、全 20 タグ）で表現し、
`pickNickname` は純粋関数のまま。話題カード（`TOPIC_CARDS`）は経由していない
— カードとタグがほぼ 1 対 1 で結果が変わらないうえ、テーブルが二重管理になるため。

### 連想元のタグを 1 つ選ぶ

本人のタグのうち **卓の他の人が持っていないもの**を優先し、そこから引く。
全部が卓で被っていれば本人のタグ全部を候補にする。
「ゲーム好きが 3 人いる卓で 3 人ともゲーム由来の名前になる」のを避けるため。

### 候補ラダー

上から順に段を作り、まだ使われていない名前が 1 つでもある段で決める。
1 段目ほど連想が濃く、下に行くほど組み合わせ数が増えるので枯れない。

| 段 | 組み合わせ | 例（alcohol + camping） | `tagId` |
|---|---|---|---|
| 1 | 由来タグの形容 × 由来タグの名詞 | ほろよいとっくり | 由来タグ |
| 2 | 本人の他タグの形容 × 由来タグの名詞 | ひをおこすとっくり | 由来タグ |
| 3 | 汎用形容 × 由来タグの名詞 ／ 由来タグの形容 × 汎用名詞 | ごきげんとっくり／ほろよいラッコ | 由来タグ |
| 4 | 汎用形容 × 汎用名詞（＝従来の 144 通り） | たそがれカピバラ | なし |
| 5 | 上記が全滅したときの base + 連番（従来の保険） | たそがれカピバラ2 | なし |

戻り値は `NicknameChoice = { name, tagId? }`。1〜3 段で決まったときだけ `tagId` が入る。
`taken`（卓で使用済みのあだ名）を避けるのと 20 文字（`NICKNAME_MAX`）に収めるのは従来どおり。

### 命名の発話で由来を明かす

`namingTagId` が載っているときは `TAGGED_NAMING_TEXTS` を使う。

```
読書がお好きとのことなので「よふかしフクロウ」と呼びますね。気に入らなかったら言ってください
```

「よふかしフクロウ」だけでは偶然に見えるが、由来を添えるとその場で話しかけるとっかかりになる
（§3.11 用途1）。タグ表示名は `hobby_tags.ts` に足した `hobbyTagLabel(id)` で引く
（ID→表示名の正本を bot 側に写さない）。タグ無しの人は従来の `NAMING_TEXTS` のまま。

### 配線（`rooms.ts`）

`doJoin` は `pickNickname` を呼んだ **あと**に `validateHobbyTags` を呼んでいたので、
タグの検証をあだ名決定より前に移した。**挙動差**: 「あだ名もタグも両方不正」なとき、
先に返るエラーがニックネーム側からタグ側へ変わる（どちらも `INVALID_INPUT`）。

`BotContext.commonTags` は触っていない。しゅんぴは `rooms.ts` から直接呼ばれる純粋関数で、
bot 側に個人タグを流す経路は要らない（ぐっちーの話題選びは今のまま）。

### 残した穴

相席（`createMatchRoom`）経由の命名は `playerJoined` に `assignedNickname` を渡していないため、
**しゅんぴの発話が出ない**（あだ名だけ静かに付く）。今回はあだ名の中身を揃えるのが目的なので
直していない。あだ名そのものはこの経路でもタグ由来になる。§7 に積む。

## 6. テスト

- `server/tests/bot_client_test.ts`（新規 15 件）… 偽 DOM に bot.js を載せて検証
  - トグル3体・ホストのみ操作可・楽観更新しない
  - 川柳テロップ（三句・モーラ数・詠み手）・字余り字足らずのラベル出し分け
  - **あだ名を textContent で入れる／`innerHTML` を使わない**（§3.8）
  - ゲーム提案から `selectGame`・アンケートへの投票と投票し直し
  - `botPollClosed` で結果表示とタイマー解放・別 pollId の無視
  - 再接続でアンケートとカードが戻る・キックで状態を捨てる
  - **音声合成 API に触れていないこと**（§3.10 の回帰よけ）
- `server/tests/senryu_test.ts` +4 … `createSenryuDetector` の遅延ロード・漢字混じり検出・`kuromoji: false` の逃げ道
- `server/tests/main_test.ts` +4 … `useKuromojiSenryu()` の値解釈3件と、`startServer` 経由で倒したときの挙動
- `server/tests/bot_test.ts`（`pickNickname` を 9 件に拡充）… タグ1個の連想・タグ2個の混成・
  卓で被ったタグを由来にしない・タグ無しは汎用プール・段の降り方・同タグ100人ぶんの一意性、
  および `namingTagId` を載せたときの文面切り替え（§5.6）
- `server/tests/hobby_tags_test.ts` +3 … `TAG_NICKNAME_WORDS` が全 20 タグを埋めていること・
  最長どうしの組み合わせが `NICKNAME_MAX` に収まること・`hobbyTagLabel` の対応
- `server/tests/player_tags_test.ts` +4 … 卓経由（あだ名を省いた `join` → 命名発話）の結合

`deno test -A` → **364 passed / 0 failed**。

kuromoji を**有効にした状態**の end-to-end（`startServer` + 環境変数なしで漢字を拾うこと）は
自動テストに入れていない。CI で辞書をもう一度読み込むと1プロセスで +300MB 級が二重になるため。
代わりに手で確認した（既定: 漢字混じり検出 1・RSS 57→384MB / `=0`: 検出 0・RSS 57→59MB）。

## 7. 未決・宿題

- **全体設計書 §2 の担当表の記述が誤っている**: 「`bot.js` … しゅんぴボットの表示・**音声**」とあるが、
  §3 の「bot の発話はチャットのみ。音声合成・読み上げは実装しない」が正。
  担当表から「音声」を削ってほしい（担当外ファイルなので未修正）
- 全体設計書 §2 のファイル一覧に `public/room/voice.js` が載っていない（同上）
- 川柳テロップの演出（フェードイン・一定時間で消す）は未実装。いまは次のカードが来るまで出しっぱなし
- `en.css` の `bot-*` に4クラス未定義（§4 末尾）。体裁のみで急ぎではない
- ~~**ぐっちーの話題カードのタグ照合が効いていない**~~ … 卓上の趣味タグ（§3.11 用途1）を
  実装したときに合わせて配線した。`rooms.ts` の `botContext` は
  `commonTags: this.commonTags(entry)` を渡すようになっている。
  「共通」の定義は**接続中の2人以上が持っているタグ**（多い順）。全員一致にすると
  1人5個までのタグで重なる場面がほとんど無く、話題カードが永久に汎用のままになるため。
  接続中が1人だけの卓では、その人のタグをそのまま使う。
  検証は `server/tests/player_tags_test.ts`（用途4の3件）
- **相席（`createMatchRoom`）でしゅんぴの命名発話が出ない**（§5.6）。`playerJoined` に
  `assignedNickname` を渡していないため、あだ名だけ静かに付く。既存の穴で、あだ名の中身は
  タグ由来になっている
- **`BOT_TICK_MS` を 15〜30 秒に縮める依頼**（§5.5）。`server/rooms.ts:132` は担当外。
  60 秒周期のままだと `SILENCE_MS = 30_000` が額面どおりには効かない
- **`LOBBY_QUIET_MS` の扱いを決める**（§5.5）。いまは `lobbyStale` がほぼ `stuck` に
  吸収されている。短くして活かすか、枝ごと畳むか
- **kuromoji の常駐メモリについてチームの ack**（§3）。仕様書 §6 に見積りを書き出したので、そこへの承認をもらう
- ack が取れたら `deno.json` の imports へ `npm:kuromoji@0.1.2` を移す（§3 の宿題）
