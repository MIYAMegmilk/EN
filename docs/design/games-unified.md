...
**レート枠の所在**（**実測**: `server/main.ts`）: `gameEvent` は `rtcSignal` と同じ構造の
**専用枠**で数える（`gameEventLimiter` / `gameEventHardLimiter`）。一般枠（`WS_RATE_MAX` = 20 件毎秒、
他の操作と共用）には落ちない。ここが一般枠のままだと、アクションゲームの正当な送信で
プレイヤーが切断される。

##### 配信量 — 実測値と、そこから来る制約

この経路は `clientEvent` 1件につき `viewChanged` を1回返し、ルーム層が
**接続数ぶんの `gameView`** を配る（`rooms.ts` `broadcastPhase`）。
つまり配信量は「view 1通の大きさ × 中継の件数 × 卓の人数」で効く。
お絵かき（`draw.ts`）は同じ構造で 1ルーム **0.5MB/秒** 前後に達しており、
「毎フレーム送るゲーム」を書けば簡単に破綻する。
...
1. **中継は「人に見せたい結果」だけに使う。** 進行そのものは `seed` から各自が導く（§2.5）。
   reflex の移植でホスト役と往復通信が丸ごと消えたのがこの効果（§6.1）。
2. **毎フレーム・毎秒何十回の送信をしない。** アニメーションはクライアントの rAF で回す（§2.4）。
...
- 移植後（**実測**: `public/room/games/mogura.js`）: 盤面は**各自バラバラのまま**にした。
  移植元がそうであり、1人プレイとして成立させるのが眼目のゲームだからである
  （`Math.random()` はクライアント側なので使ってよい。禁止なのはサーバーモジュール）。
  スコアは各自のクライアントが数え、終了時に relay で1件だけ申告して一覧表示する
  （公式スコアには乗らない）。
  「全員同じ盤で競う」ようにしたければ `createRng(view.seed)` に差し替えるだけでよい
  ——実際、絵合わせ（emoawase）はそちらを選んでいる。
